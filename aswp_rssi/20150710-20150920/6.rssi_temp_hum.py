...
#	print link[0] + "->" + link[1]
	...
	## Draw a figure about the temp_<rssi_std>
	fig_t = plt.plot(p_temp, p_rssi_std, 'go')
	
	plt.ylabel("RSSI_Std / dBm")
	plt.xlabel("Temperature / $^\circ$C")
	...
	extra_max = Rectangle((0, 0), 1, 1, fc="w", fill=False, edgecolor='none', 
			linewidth=0)	
	extra_max_label = "max_std: " + str("{0:.2f}".format(np.max(p_rssi_std))) + " dBm"
	
	extra_min = Rectangle((0, 0), 1, 1, fc="w", fill=False, edgecolor='none', 
			linewidth=0)
	extra_min_label = "min_std: " + str("{0:.2f}".format(np.min(p_rssi_std))) + " dBm"
	...
	plt.axis([min(p_temp) - 1, 48, min(p_rssi_std) - 0.5, max(p_rssi_std) + 0.5])
	
	# draw a line on temperature 35
	plt.plot((35, 35), (min(p_rssi_std) - 0.5, max(p_rssi_std) + 0.5), 'b-')
	

	plt.savefig("figures\\std_frssi_temp\\" + nodes_info[link[0]] + "_" + str(total_rssi_avg) + "_" 
				+str(int(min(p_temp))) + "_" + link[0] + "_" + link[1] + ".png")
	plt.clf()
...
#	plt.plot((35, 35), (min(p_rssi) - 0.5, max(p_rssi) + 0.5), 'b-')
	...
	# draw figure about std
	fig_t = plt.plot(p_hum, p_rssi_std, 'bo')
	plt.ylabel("RSSI_Std / dBm")
	plt.xlabel("Humidity / %")
	...
	plt.legend([extra_max, extra_min, extra_avg, extra_std], 
				[extra_max_label, extra_min_label, extra_avg_label, extra_std_label]
				, loc='best')
	plt.axis([min(p_hum) - 1, max(p_hum) + 1, min(p_rssi_std) - 0.5,  max(p_rssi_std)+ 0.5])
	
	# draw a line on temperature 35
#	plt.plot((35, 35), (min(p_rssi) - 0.5, max(p_rssi) + 0.5), 'b-')
	
	plt.savefig("figures\\std_frssi_hum\\"   + nodes_info[link[0]] + "_" + str(total_rssi_avg) + "_" 
				+str(int(min(p_hum))) + "_" + link[0] + "_" + link[1] + ".png")
	plt.clf()
...
#			print "+++++++++++cur temp_idx is: " + str(temp_idx)
	...
	for T in T_hum_frssi:
		...
		## actuall this should call plot_hum_rssi(hum_frssi_list, link)
...
#		continue
	...
	if not os.path.exists("figures\\std_frssi_temp"):
		os.makedirs("figures\\std_frssi_temp\\")
	
	if not os.path.exists("figures\\std_frssi_hum"):
		os.makedirs("figures\\std_frssi_hum\\")
	...
	plot_temp_rssi(temp_frssi, t, "figures\\frssi_temp")
	plot_hum_rssi(hum_frssi, t, "figures\\frssi_hum")
	
	print t[0] + " -> " + t[1]
	
#	if t[0] == "10011" and t[1] == "1":
		# plot figure for link 10011_1
#	plot_temp_hum_rssi(temp_hum_frssi, t, "figures\\temp_hum_frssi")
