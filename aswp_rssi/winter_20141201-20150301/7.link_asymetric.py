'''
analyze the asymetric of each link.
1. get diff and brssi
2. get diff = diff - brssi
3. draw temp_diff figure for each link

# very similar to script 6.
...
'''
	### compute average temp and diff
def plot_temp_rssi(temp_rssi_list, link, dir):
	temp_rssi_list.sort()
	...
	for i in range(0, len(temp_rssi_list)):
		# for each <temp, diff> pairs, average them for every 0.5 C
	...
	#		print "+++++ temp_rssi_list is: " + str(tmp_rssi_list)
			...
			avg_temp_diff.append([avg_temp, avg_rssi])
			
			if avg_temp != 0 and avg_temp > -20:
	...
	#### plot figure
	if len(p_rssi) <= 0:
		print "------------not temp figure for: " + str(link)
		print "\t\treadings are: " + str(temp_rssi_list)		
		return 
...
#	print link[0] + "->" + link[1]
	...
	extra_max = Rectangle((0, 0), 1, 1, fc="w", fill=False, edgecolor='none', 
			linewidth=0)	
	extra_max_label = "max_diff: " + str("{0:.2f}".format(np.max(p_rssi))) + " dBm"
	
	extra_min = Rectangle((0, 0), 1, 1, fc="w", fill=False, edgecolor='none', 
			linewidth=0)
	extra_min_label = "min_diff: " + str("{0:.2f}".format(np.min(p_rssi))) + " dBm"
	
	extra_avg = Rectangle((0, 0), 1, 1, fc="w", fill=False, edgecolor='none', 
			linewidth=0)
	extra_avg_label = "avg_diff: " + str("{0:.2f}".format(np.mean(p_rssi))) + " dBm"
	...
	plt.plot((35, 35), (-5, 10), 'b-')
	
	# "-56_10011_1_.png"
	plt.savefig(dir + "_unified\\" + nodes_info[link[0]] + "_" + str(total_rssi_avg) + "_" 
	...
	plt.savefig(dir + "\\"   + nodes_info[link[0]] + "_" + str(total_rssi_avg) + "_" 
				+str(int(min(p_temp))) + "_" + link[0] + "_" + link[1] + ".png")
	plt.clf()
	...
	### compute average hum and diff
def plot_hum_rssi(hum_rssi_list, link, dir):
	...
	for i in range(0, len(hum_rssi_list)):
		# for each <hum, diff> pairs, average them for every 1%
	...
	#			avg_rssi += p[1]*1.0/len(tmp_list)
			...
			avg_hum_diff.append([avg_hum, avg_rssi])
	...
	if len(p_rssi) <= 0:
		print "------------not hum figure for: " + str(link)
		print "\t\treadings are: " + str(hum_rssi_list)
		return
	total_rssi_avg = int(total_rssi_avg/len(p_rssi))
...
###
def plot_temp_hum_rssi(temp_hum_rssi_list, link, dir):
...
#	total_rssi_avg = 0
	...
	for i in range(0, len(temp_hum_rssi_list)):
		# for each <temp, hum, diff> pairs, get <hum, diff> pair for every temperature
...
#			print "+++++++++++cur temp_idx is: " + str(temp_idx)
			...
			# store hum and diff pairs for this temperature
			if not T_hum_diff.has_key(str(temp_idx*5)):
				T_hum_diff[str(temp_idx*5)] = []
			T_hum_diff[str(temp_idx*5)].append([temp_hum_rssi_list[i][1], temp_hum_rssi_list[i][2]])
	
	
	for T in T_hum_diff:
		# for each temperature, draw figures about <hum, diff>
		hum_diff_list = T_hum_diff[T]		# get the <hum, diff> pairs
		
		## actuall this should call plot_hum_rssi(hum_diff_list, link)
		## however, the path is different, the figures are also slightly different
		
		hum_diff_list.sort()
		pre_hum = hum_diff_list[0][0]
		tmp_rssi_list = []
		tmp_hum_list = []
...
#			print "\ttemp is: " + T 
#			print "\t" + str(hum_diff_list)
			
		for i in range(0, len(hum_diff_list)):
			
			# for each <hum, diff> pairs, average them for every 1%
			if hum_diff_list[i][0] - pre_hum >= 0.25:
				# Reach 0.5%, compute the average
				avg_rssi = 0.0
				avg_hum = 0.0
				rssi_std = 0.0
...
#						print "\p_rssi is: " + str(p_rssi)
				...
				tmp_hum_list.append(hum_diff_list[i][0])
				tmp_rssi_list.append(hum_diff_list[i][1])
		...
		if len(p_rssi) <= 0:
			print "------------no temp_hum figure for: " + str(link)
			print "\t\treadings are: " + str(hum_diff_list)	
			continue
...
#		continue
	...
	if not os.path.exists("figures\\diff_hum"):
		os.makedirs("figures\\diff_hum")
	if not os.path.exists("figures\\diff_hum_unified"):
		os.makedirs("figures\\diff_hum_unified")	
		
	if not os.path.exists("figures\\diff_temp"):
		os.makedirs("figures\\diff_temp")
	if not os.path.exists("figures\\diff_temp_unified"):
		os.makedirs("figures\\diff_temp_unified")
	
	if not os.path.exists("figures\\diff_temp_hum"):
		os.makedirs("figures\\diff_temp_hum")
	if not os.path.exists("figures\\diff_temp_hum_unified"):
		os.makedirs("figures\\diff_temp_hum_unified")
	...
	for line in lines:
		...
		if line_count > 1:
			s = line.split()
			if len(s) == DATA_LEN :
				if float(s[F_RSSI]) == -97 or float(s[B_RSSI]) == -97:
					# if the value is -97, then the IRIS mote didn't set the rssi correctly
					continue
				temp_diff.append([float(s[TEMP]), float(s[F_RSSI]) - float(s[B_RSSI])])
				hum_diff.append([float(s[HUM]), float(s[F_RSSI]) - float(s[B_RSSI])])
				temp_hum_diff.append([float(s[TEMP]), float(s[HUM]), float(s[F_RSSI]) - float(s[B_RSSI])])
	
	plot_temp_rssi(temp_diff, t, "figures\\diff_temp")
	plot_hum_rssi(hum_diff, t, "figures\\diff_hum")
	
	print t[0] + " -> " + t[1]
	
#	if t[0] == "10011" and t[1] == "1":
		# plot figure for link 10011_1
	plot_temp_hum_rssi(temp_hum_diff, t, "figures\\diff_temp_hum")
